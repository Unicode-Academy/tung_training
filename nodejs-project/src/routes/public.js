...
router.post(
  "/cart/updateCart",
  Auth.ensureAuthenticated,
  publicController.updateCart
);
router.get(
  "/checkout/:step",
  Auth.ensureAuthenticated,
  publicController.goToCheckout
);
router.post(
  "/checkout/1",
  Auth.ensureAuthenticated,
  publicController.validateShipping
);
router.post(
  "/checkout/2",
  Auth.ensureAuthenticated,
  publicController.validatePayment
);
router.post(
  "/checkout/3",
  Auth.ensureAuthenticated,
  publicController.confirmOrder
);

router.get("/test4", publicController.test);
